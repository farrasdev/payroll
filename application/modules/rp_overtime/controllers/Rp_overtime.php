...
class Rp_overtime extends MY_Controller
{
  ...
  public function detail($id = null)
  {
    ...
    $config['total_rows'] = $this->m_rp_overtime->detail_total($id, $data['search'])['total'];
    $this->pagination->initialize($config);

    $data['cur_page'] = $this->uri->segment(4, 0);
    $data['main'] = $this->m_rp_overtime->detail($id, $data['cur_page'], $config['per_page'], $data['search']);
    $data['payroll'] = $this->m_rp_overtime->by_field('payroll_id', $id);
    $data['id'] = $id;
    $data['menu'] = $this->menu;
  ...
  {
    $main = $this->m_rp_overtime->detail_all($id);
