          ...
          <div class="card">
            ...
            <form id="form" action="<?= site_url() . '/' . $menu['controller'] . '/save/' . $id ?>" method="post" autocomplete="off" enctype="multipart/form-data">
              <div class="card-body">
                ...
                <input type="hidden" class="form-control form-control-sm" name="comm_trans_all_id" id="comm_trans_all_id" value="<?= @$main['comm_trans_all_id'] ?>" required>
                <?php if ($id != null) : ?>
                  <input type="hidden" class="form-control form-control-sm" name="old" id="old" value="<?= @$main['comm_trans_all_id'] ?>" required>
                <?php endif; ?>
                ...
                <div class="form-group row">
                  ...
                  <div class="col-sm-2">
                    <input type="text" class="form-control form-control-sm datepicker" name="comm_trans_all_date" id="comm_trans_all_date" value="<?= @reverse_date($main['comm_trans_all_date']) ?>" required>
                  </div>
                </div>
                <div class="form-group row">
      ...
      rules: {
        comm_trans_all_id: {
          remote: {
            ...
            data: {
              'comm_trans_all_id': function() {
                return $('#comm_trans_all_id').val();
              }
            },
            dataType: 'json'
          }
