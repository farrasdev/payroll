...
class Employee_status extends MY_Controller
{
  ...
  function __construct()
  {
    ...
    if ($this->cookie['order'] == null) $this->cookie['order'] = array('field' => 'employee_status_id', 'type' => 'asc');
  ...
  public function index()
  {
    ...
    $this->cookie['total_rows'] = $this->m_employee_status->all_rows($this->cookie);
    set_cookie_menu($this->menu_id, $this->cookie);
    //main data
    $data['menu'] = $this->menu;
    $data['cookie'] = $this->cookie;
    $data['main'] = $this->m_employee_status->list_data($this->cookie);
  ...
  public function form($id = null)
  {
    ...
    if ($id == null) {
      ...
      $data['main'] = $this->m_employee_status->by_field('employee_status_id', $id);
    }
    $data['id'] = $id;
    $data['menu'] = $this->menu;
  ...
  public function save($id = null)
  {
    ...
    $cek = $this->m_employee_status->by_field('employee_status_id', $data['employee_status_id']);
    if ($id == null) {
      if ($cek != null) {
      ...
      unset($data['old']);
      $this->m_employee_status->save($data, $id);
      create_log(2, $this->menu['menu_name']);
      $this->session->set_flashdata('flash_success', 'Kode berhasil ditambahkan.');
    } else {
      if ($data['old'] != $data['employee_status_id'] && $cek != null) {
        $this->session->set_flashdata('flash_error', 'Kode sudah ada di sistem.');
  ...
  {
    ($id == null) ? authorize($this->menu, '_create') : authorize($this->menu, '_update');
    $this->m_employee_status->delete($id);
  ...
  public function status($type = null, $id = null)
  {
    ...
    if ($type == 'enable') {
      $this->m_employee_status->update($id, array('is_active' => 1));
    } else {
      $this->m_employee_status->update($id, array('is_active' => 0));
    }
      ...
      foreach ($data['checkitem'] as $key) {
        switch ($type) {
          case 'delete':
            ...
            $this->m_employee_status->delete($key);
          ...
          case 'enable':
            authorize($this->menu, '_update');
            $this->m_employee_status->update($key, array('is_active' => 1));
          ...
          case 'disable':
            authorize($this->menu, '_update');
            $this->m_employee_status->update($key, array('is_active' => 0));
  ...
  public function ajax($type = null, $id = null)
  {
    if ($type == 'check_id') {
      ...
      if ($id == null) {
        ...
        echo ($id != $data['employee_status_id'] && $cek != null) ? 'false' : 'true';
      }
    }
  }
}
