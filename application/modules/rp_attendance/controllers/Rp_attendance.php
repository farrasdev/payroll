...
class Rp_attendance extends MY_Controller
{
  ...
  public function detail($start_date = "", $end_date = "")
  {
    if ($start_date == "" || $end_date == "") {
      ...
      $data['main'] = $this->m_rp_attendance->detail($start_date, $end_date, $data['cur_page'], $config['per_page'], $data['search']);
  ...
  public function pdf($start_date, $end_date)
  {
    ...
    $main = $this->m_rp_attendance->detail_all($start_date, $end_date);
    $profile = $this->m_profile->get_first();
    $this->load->library('pdf');
    $pdf = new Pdf('l', 'mm', array(330, 210)); //A5
    $pdf->AliasNbPages();
    $pdf->SetTitle('Kehadiran ' . date_id(reverse_date($start_date)) . ' s/d ' . date_id(reverse_date($end_date)));
    $pdf->AddPage();
