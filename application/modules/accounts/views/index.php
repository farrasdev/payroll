                    ...
                    <table class="table table-bordered table-sm table-striped table-head-fixed">
                      <thead>
                        <tr>
                          ...
                          <th class="text-center" width="100"><?= table_sort($menu['menu_id'], 'Kode', 'account_id', $cookie['order']) ?></th>
                          <th class="text-center" width="100">Induk</th>
                          <th class="text-center"><?= table_sort($menu['menu_id'], 'Nama', 'account_name', $cookie['order']) ?></th>
                          <th class="text-center" width="100">Tipe Akun</th>
                          <th class="text-center" width="100">Tipe Saldo</th>
                          <th class="text-center" width="100">Tipe Laporan</th>
                          <th class="text-center" width="100">Tipe Sub Laporan</th>
                            ...
                            foreach ($main as $r) : ?>
                              <tr>
                                ...
                                <td class="text-center">
                                  <div class="pretty p-icon">
                                    <input class="checkitem" type="checkbox" value="<?= $r['account_id'] ?>" name="checkitem[]" onclick="checkItem();" />
                                    <div class="state">
                                      <i class="icon fas fa-check"></i><label></label>
                                    </div>
                                  </div>
                                ...
                                <td class="text-center">
                                  <?php if ($menu['_update'] == 1) : ?>
                                    <a class="text-warning mr-1" href="<?= site_url() . '/' . $menu['controller'] . '/form/' . $r['account_id'] ?>"><i class="fas fa-pencil-alt"></i></a>
                                  <?php endif; ?>
                                  <?php if ($menu['_delete'] == 1) : ?>
                                    <a class="text-danger btn-delete" href="<?= site_url() . '/' . $menu['controller'] . '/delete/' . $r['account_id'] ?>"><i class="fas fa-trash-alt"></i></a>
                                  <?php endif; ?>
                                </td>
                                <td><?= $r['account_id'] ?></td>
                                <td><?= $r['parent_id'] ?></td>
                                ...
                                <td class="text-center">
                                  <?php $account_type = ['G' => 'Grup', 'D' => 'Detail'] ?>
                                  <?= $account_type[$r['account_type']] ?>
                                </td>
                                <td class="text-center">
                                  <?php $balance_type = ['D' => 'Debit', 'C' => 'Kredit'] ?>
                                  <?= $balance_type[$r['balance_type']] ?>
                                </td>
                                <td class="text-center">
                                  <?php $report_type = ['PL' => 'Laba Rugi', 'BS' => 'Neraca'] ?>
                                  <?= $report_type[$r['report_type']] ?>
                                </td>
                                <td class="text-center">
                                  <?php $sub_report_type = ['ACT' => 'Aktiva', 'PSV' => 'Pasiva', 'INC' => 'Pendapatan', 'SPN' => 'Pengeluaran'] ?>
                                  <?= $sub_report_type[$r['sub_report_type']] ?>
                                </td>
                                <td class="text-center td-status">
                                  <?php if ($menu['_update'] == 1) : ?>
                                    <?php if ($r['is_active'] == 1) : ?>
                                      <a href="<?= site_url() . '/' . $menu['controller'] . '/status/disable/' . $r['account_id'] ?>">
                                        <i class="icon-status fas fa-toggle-on text-success"></i>
                                      </a>
                                    <?php else : ?>
                                      <a href="<?= site_url() . '/' . $menu['controller'] . '/status/enable/' . $r['account_id'] ?>">
