...
class Rp_overtime_4 extends MY_Controller
{
  ...
  public function pdf($start_date, $end_date)
  {
    ...
    $main = $this->m_rp_overtime_4->detail_all($start_date, $end_date);
    $profile = $this->m_profile->get_first();
    $this->load->library('pdf');
    $pdf = new Pdf('l', 'mm', array(330, 210)); //A5
    $pdf->AliasNbPages();
    $pdf->SetTitle('Overtime 4 ' . date_id(reverse_date($start_date)) . ' s/d ' . date_id(reverse_date($end_date)));
    $pdf->AddPage();
    ...
    $pdf->Cell(0, 5, 'REKAP OVERTIME 4', 0, 1, 'C');
    $pdf->SetFont('Arial', '', 10);
    ...
    $pdf->Output('I', 'Overtime 4_' . $start_date . '_' . $end_date . '-' . date('Ymdhis') . '.pdf');
  }
}
