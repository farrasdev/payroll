                    ...
                    <table class="table table-bordered table-sm table-head-fixed">
                      <thead>
                        <tr>
                          ...
                          <th class="text-center" width="200"><?= table_sort($menu['menu_id'], 'No', 'agreement_id', $cookie['order']) ?></th>
                          <th class="text-center"><?= table_sort($menu['menu_id'], 'Nama Pegawai', 'employee_name', $cookie['order']) ?></th>
                          <th class="text-center" width=""><?= table_sort($menu['menu_id'], 'Departemen', 'department_name', $cookie['order']) ?></th>
                          <th class="text-center" width=""><?= table_sort($menu['menu_id'], 'Divisi', 'division_name', $cookie['order']) ?></th>
                          <th class="text-center" width=""><?= table_sort($menu['menu_id'], 'Posisi', 'position_name', $cookie['order']) ?></th>
                          <th class="text-center" width="90"><?= table_sort($menu['menu_id'], 'Tgl. Surat', 'entry_date', $cookie['order']) ?></th>
                        </tr>
                      </thead>
                      <?php if (@$main == null) : ?>
                        <tbody>
                            ...
                            foreach ($main as $r) : ?>
                              <tr>
                                ...
                                <td class="text-center">
                                  <div class="pretty p-icon">
                                    <input class="checkitem" type="checkbox" value="<?= $r['employee_id'] ?>" name="checkitem[]" onclick="checkItem();" />
                                    <div class="state">
                                      <i class="icon fas fa-check"></i><label></label>
                                    </div>
                                  </div>
                                ...
                                <td class="text-center">
                                  ...
                                  <?php if ($menu['_delete'] == 1) : ?>
                                    <a class="text-danger btn-delete" href="<?= site_url() . '/' . $menu['controller'] . '/delete/' . $r['agreement_id'] ?>"><i class="fas fa-trash-alt"></i></a>
                                  <?php endif; ?>
                                </td>
                                <td class="text-center"><?= $r['agreement_number'] ?></td>
                                ...
                                <td class="text-center"><?= reverse_date($r['agreement_date']) ?></td>
                              </tr>
                            <?php endforeach; ?>
                          </form>
                        </tbody>
