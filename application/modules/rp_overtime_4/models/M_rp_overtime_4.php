...
class M_rp_overtime_4 extends CI_Model
{
  ...
  public function detail($start_date, $end_date, $limit, $offset, $search)
  {
    ...
    foreach ($data as $k => $v) {
      ...
      for ($i = 0; $i <= $diff; $i++) {
        ...
        $att = $this->db->query("SELECT overtime_4 FROM attendance WHERE employee_id = '" . $v['employee_id'] . "' AND attendance_date = '" . $cur_date . "'")->row_array();
        if ($att != null) {
          $attendance[$cur_date] = $att['overtime_4'];
        }
      }
