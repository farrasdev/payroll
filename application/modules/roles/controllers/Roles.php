...
class Roles extends MY_Controller
{
  ...
  public function index()
  {
    ...
    $data['main'] = $this->m_roles->list_data($this->cookie);
  ...
  public function form($id = null)
  {
    ...
    if ($id == null) {
      ...
      $data['main'] = $this->m_roles->by_field('role_id', $id);
    }
    $data['id'] = $id;
    $data['menu'] = $this->menu;
  ...
  public function save($id = null)
  {
    ...
    $cek = $this->m_roles->by_field('role_id', $data['role_id']);
    if ($id == null) {
      if ($cek != null) {
      ...
      if ($data['old'] != $data['role_id'] && $cek != null) {
        $this->session->set_flashdata('flash_error', 'Kode sudah ada di sistem.');
  ...
  public function status($type = null, $id = null)
  {
    ...
    if ($type == 'enable') {
      $this->m_roles->update($id, array('is_active' => 1));
    } else {
      $this->m_roles->update($id, array('is_active' => 0));
    }
    ...
    if (isset($data['checkitem'])) {
      foreach ($data['checkitem'] as $key) {
        switch ($type) {
          ...
          case 'enable':
            authorize($this->menu, '_update');
            $this->m_roles->update($key, array('is_active' => 1));
          ...
          case 'disable':
            authorize($this->menu, '_update');
            $this->m_roles->update($key, array('is_active' => 0));
  ...
  public function authorization($id = null)
  {
    ...
    $data['id'] = $id;
    $data['menu'] = $this->menu;
    $data['menu_list'] = $this->m_roles->menu_list($id);
  ...
  public function authorization_save($id)
  {
    ...
    $this->m_roles->authorization_save($data, $id);
