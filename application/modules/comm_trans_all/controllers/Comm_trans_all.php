...
class Comm_trans_all extends MY_Controller
{
  ...
  function __construct()
  {
    ...
    if ($this->cookie['order'] == null) $this->cookie['order'] = array('field' => 'comm_trans_all_date', 'type' => 'desc');
  ...
  public function index()
  {
    ...
    $this->cookie['total_rows'] = $this->m_comm_trans_all->all_rows($this->cookie);
    set_cookie_menu($this->menu_id, $this->cookie);
    //main data
    $data['menu'] = $this->menu;
    $data['cookie'] = $this->cookie;
    $data['main'] = $this->m_comm_trans_all->list_data($this->cookie);
  ...
  public function form($id = null)
  {
    ...
    if ($id == null) {
      ...
      $data['main'] = $this->m_comm_trans_all->by_field('comm_trans_all_id', $id);
    }
    $data['id'] = $id;
    $data['menu'] = $this->menu;
  ...
  public function save($id = null)
  {
    ...
    $data['comm_trans_all_date'] = reverse_date($data['comm_trans_all_date']);
    $cek = $this->m_comm_trans_all->by_field('comm_trans_all_id', $data['comm_trans_all_id']);
    if ($id == null) {
      if ($cek != null) {
        $this->session->set_flashdata('flash_error', 'Kode sudah ada di sistem.');
        redirect(site_url() . '/' .  $this->menu['controller']  . '/form/');
      }
      $data['comm_trans_all_id'] = $this->uuid->v4();
      unset($data['old']);
      $this->m_comm_trans_all->save($data, $id);
      create_log(2, $this->menu['menu_name']);
      $this->session->set_flashdata('flash_success', 'Kode berhasil ditambahkan.');
    } else {
      if ($data['old'] != $data['comm_trans_all_id'] && $cek != null) {
        $this->session->set_flashdata('flash_error', 'Kode sudah ada di sistem.');
  ...
  {
    ($id == null) ? authorize($this->menu, '_create') : authorize($this->menu, '_update');
    $this->m_comm_trans_all->delete($id);
  ...
  public function status($type = null, $id = null)
  {
    ...
    if ($type == 'enable') {
      $this->m_comm_trans_all->update($id, array('is_active' => 1));
    } else {
      $this->m_comm_trans_all->update($id, array('is_active' => 0));
    }
      ...
      foreach ($data['checkitem'] as $key) {
        switch ($type) {
          case 'delete':
            ...
            $this->m_comm_trans_all->delete($key);
          ...
          case 'enable':
            authorize($this->menu, '_update');
            $this->m_comm_trans_all->update($key, array('is_active' => 1));
          ...
          case 'disable':
            authorize($this->menu, '_update');
            $this->m_comm_trans_all->update($key, array('is_active' => 0));
  ...
  public function ajax($type = null, $id = null)
  {
    if ($type == 'check_id') {
      ...
      if ($id == null) {
        ...
        echo ($id != $data['comm_trans_all_id'] && $cek != null) ? 'false' : 'true';
      }
    }
  }
}
