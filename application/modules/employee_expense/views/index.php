                    ...
                    <table class="table table-bordered table-sm table-striped table-head-fixed">
                      <thead>
                        <tr>
                          ...
                          <th class="text-center" width="100"><?= table_sort($menu['menu_id'], 'Tanggal', 'employee_expense_date', $cookie['order']) ?></th>
                            ...
                            foreach ($main as $r) : ?>
                              <tr>
                                ...
                                <td class="text-center">
                                  <div class="pretty p-icon">
                                    <input class="checkitem" type="checkbox" value="<?= $r['employee_expense_id'] ?>" name="checkitem[]" onclick="checkItem();" />
                                    <div class="state">
                                      <i class="icon fas fa-check"></i><label></label>
                                    </div>
                                  </div>
                                ...
                                <td class="text-center">
                                  <?php if ($menu['_update'] == 1) : ?>
                                    <a class="text-warning mr-1" href="<?= site_url() . '/' . $menu['controller'] . '/form/' . $r['employee_expense_id'] ?>"><i class="fas fa-pencil-alt"></i></a>
                                  <?php endif; ?>
                                  <?php if ($menu['_delete'] == 1) : ?>
                                    <a class="text-danger btn-delete" href="<?= site_url() . '/' . $menu['controller'] . '/delete/' . $r['employee_expense_id'] ?>"><i class="fas fa-trash-alt"></i></a>
                                  <?php endif; ?>
                                </td>
                                <td class="text-center"><?= $r['employee_expense_date'] ?></td>
                                <td class="text-center text-bold"><?= $r['employee_id'] ?></td>
                                <td><?= $r['employee_name'] ?></td>
                                <td><?= $r['description'] ?></td>
                                <td class="text-right"><?= num_id($r['nominal']) ?></td>
                              </tr>
                            <?php endforeach; ?>
                          </form>
                        </tbody>
