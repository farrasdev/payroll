...
class Positions extends MY_Controller
{
  ...
  function __construct()
  {
    ...
    if ($this->cookie['order'] == null) $this->cookie['order'] = array('field' => 'position_name', 'type' => 'asc');
  ...
  public function index()
  {
    ...
    $this->cookie['total_rows'] = $this->m_positions->all_rows($this->cookie);
    set_cookie_menu($this->menu_id, $this->cookie);
    //main data
    $data['menu'] = $this->menu;
    $data['cookie'] = $this->cookie;
    $data['main'] = $this->m_positions->list_data($this->cookie);
  ...
  public function form($id = null)
  {
    ...
    if ($id == null) {
      ...
      $data['main'] = $this->m_positions->by_field('position_id', $id);
    }
    $data['id'] = $id;
    $data['menu'] = $this->menu;
  ...
  public function status($type = null, $id = null)
  {
    ...
    if ($type == 'enable') {
      $this->m_positions->update($id, array('is_active' => 1));
    } else {
      $this->m_positions->update($id, array('is_active' => 0));
    }
    ...
    if (isset($data['checkitem'])) {
      foreach ($data['checkitem'] as $key) {
        switch ($type) {
          ...
          case 'enable':
            authorize($this->menu, '_update');
            $this->m_positions->update($key, array('is_active' => 1));
          ...
          case 'disable':
            authorize($this->menu, '_update');
            $this->m_positions->update($key, array('is_active' => 0));
