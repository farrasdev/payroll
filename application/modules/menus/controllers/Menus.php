...
class Menus extends MY_Controller
{
  ...
  public function index()
  {
    ...
    $this->cookie['total_rows'] = $this->m_menus->all_rows($this->cookie);
    set_cookie_menu($this->menu_id, $this->cookie);
    //main data
    $data['menu'] = $this->menu;
    $data['cookie'] = $this->cookie;
    $data['main'] = $this->m_menus->list_data($this->cookie);
  ...
  public function form($id = null)
  {
    ...
    if ($id == null) {
      ...
      $data['main'] = $this->m_menus->by_field('menu_id', $id);
    }
    $data['id'] = $id;
    $data['menu'] = $this->menu;
    $data['all'] = $this->m_menus->all_data();
  ...
  public function save($id = null)
  {
    ...
    $cek = $this->m_menus->by_field('menu_id', $data['menu_id']);
    if ($id == null) {
  ...
  public function status($type = null, $id = null)
  {
    ...
    if ($type == 'enable') {
      $this->m_menus->update($id, array('is_active' => 1));
    } else {
      $this->m_menus->update($id, array('is_active' => 0));
    }
    ...
    if (isset($data['checkitem'])) {
      foreach ($data['checkitem'] as $key) {
        switch ($type) {
          ...
          case 'enable':
            authorize($this->menu, '_update');
            $this->m_menus->update($key, array('is_active' => 1));
          ...
          case 'disable':
            authorize($this->menu, '_update');
            $this->m_menus->update($key, array('is_active' => 0));
